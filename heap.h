...
public:
	...
	heap(const DynamicArray<T>& table) :DynamicArray<T>(table)
	{
		/*for (long long i = this->get_current_size(); i >= 0; i--)
		{
			this->heap_up(i,cmp1);
	...
	heap(const DynamicArray<T>& table, bool (*fun)(T const, T const)) :DynamicArray<T>(table)
	{
		/*for (long long i = this->get_current_size(); i >= 0; i--)
		{
			this->heap_up(i,cmp1);
...
public:
	...
	//	else this->inplace_sort(cmp1);
	//};
	heap_sort_inplace(const DynamicArray<T>& heap1, bool (*fun)(T const, T const)) :heap<T>(heap1,fun)
	{
